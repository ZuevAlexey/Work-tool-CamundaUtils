...
namespace CamundaUtils {
    ...
    public partial class MainWindow {
        ...
        private void CopyButton_Click(object sender, RoutedEventArgs e) {
            ...
            if(btn == ProcessIdCopyButton) Clipboard.SetText(ProcessIdTextBox.Text);
        }
        ...
        private static async Task BlockAction(UIElement control, Func<Task> action) {
            control.IsEnabled = false;
            await DoWithErrorLog(action);
            control.IsEnabled = true;
        }

        private static async Task DoWithErrorLog(Func<Task> action) {
            try {
                await action();
            }
            catch(Exception ex) {
